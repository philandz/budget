//! PNJ gold price provider stub.
//!
//! Same licensing caveats as `sjc` and `doji`. Disabled by default;
//! enable only after commercial agreement via `PORTFOLIO_ENABLE_PNJ=1`.
...
impl PnjProvider {
    ...
    fn enabled() -> bool {
        std::env::var("PORTFOLIO_ENABLE_PNJ")
            .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
            .unwrap_or(false)
    }
}

impl Default for PnjProvider {
    fn default() -> Self {
        Self::new()
    }
}

impl PriceProvider for PnjProvider {
    ...
    fn supports(&self, asset: &pb::PortfolioAsset) -> bool {
        ...
        match &asset.details {
            Some(pb::portfolio_asset::Details::GoldLot(g)) => {
                matches!(
                    g.purity,
                    x if x == pb::GoldPurity::Pnj999 as i32
    ...
    fn fetch<'a>(
        ...
        if !Self::enabled() {
            tracing::debug!("pnj provider disabled by config");
        } else {
            tracing::warn!("pnj provider enabled but no implementation yet — returning empty");
        }
...
mod tests {
    ...
    #[test]
    fn name_is_pnj() {
        assert_eq!(PnjProvider::new().name(), "pnj");
    }

    #[test]
    fn supports_returns_false_when_disabled() {
        std::env::remove_var("PORTFOLIO_ENABLE_PNJ");
        assert!(!PnjProvider::new().supports(&gold_asset(pb::GoldPurity::Pnj999 as i32)));
    }

    #[test]
    #[serial_test::serial]
    fn supports_returns_true_when_enabled() {
        std::env::set_var("PORTFOLIO_ENABLE_PNJ", "1");
        let p = PnjProvider::new();
